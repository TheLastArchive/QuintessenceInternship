...
namespace DamData
{
    class Commands
    {
        Dictionary<string, Dam> damObjects;

        public Commands(Dictionary<string, Dam> damObjects) => this.damObjects = damObjects;
        ...
        {
            foreach (var entry in damObjects)
                damObjects[entry.Key].PrintAllDamData();
            return true;
        }
        ...
        private bool PrintDam(string damName, string date)
        {
            try
            {
                damObjects[damName.ToUpper()].PrintDam(FormatDate(date));
                
            }
            catch (KeyNotFoundException)
            {
                Console.WriteLine("{0} not found", damName);
            }
        ...
        private bool TestCommand()
        {
            ...
            timer.Stop();
            Console.WriteLine("Dictionary Execution time: " + timer.Elapsed);
